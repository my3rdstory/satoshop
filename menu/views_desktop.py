...
def create_cart_invoice_desktop(request, store_id):
    ...
    try:
        ...
        logger.debug(f"[DESKTOP] 인보이스 생성 성공 - 세션에 임시 저장됨")
...
@csrf_exempt
@require_POST
def process_free_order_desktop(request, store_id):
    """데스크톱용 무료 상품 주문 처리"""
    try:
        # 스토어 조회 (비회원도 접근 가능)
        store = get_object_or_404(Store, store_id=store_id, is_active=True)
        ...
        logger.debug(f"[DESKTOP] 무료 상품 주문 요청 - 스토어: {store.store_name}, 아이템 수: {len(cart_items)}")
        ...
        with transaction.atomic():
            ...
            logger.debug(f"[DESKTOP] 무료 상품 주문 생성 완료 - 주문번호: {menu_order.order_number}")
        ...
        logger.error(f"[DESKTOP] 무료 상품 주문 처리 오류: {str(e)}")
