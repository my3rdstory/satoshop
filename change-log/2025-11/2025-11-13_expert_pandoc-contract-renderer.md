...
- Render 빌드 환경에서는 apt를 사용하지 않고, `build.sh`가 `expert/deps/sources/`에 둔 Pandoc·TinyTeX 압축본을 자동으로 해제해 사용하고 `expert/fonts/`에 둔 Noto Sans KR TTF를 경로에 추가합니다. 압축본만 버전 관리하면 되므로 GitHub 제한을 넘지 않습니다.
