...
- Render 빌드 환경에서는 기본적으로 루트 권한이 제공되므로 `build.sh`에서 직접 `apt-get install`을 수행하되 `/tmp` 하위 임시 state/log 디렉터리를 사용해 읽기 전용 루트에서도 `pandoc/xelatex/fonts-noto-cjk`를 설치하도록 고정했습니다.
