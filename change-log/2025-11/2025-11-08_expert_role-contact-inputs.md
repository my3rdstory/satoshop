...
- 계약서 PDF를 시스템 CJK 폰트(DejaVuSans)로 렌더링해 한글이 깨지지 않도록 했습니다.
...
- 서명 타임라인 표시를 시:분:초 형식으로 확장하고, 손상된 PDF 첨부가 있어도 병합이 중단되지 않도록 오류를 무시하며 나머지 파일을 합칩니다.
- DirectContractDocument의 서명 이미지와 최종 PDF가 항상 S3 스토리지를 사용하도록 모델·저장 로직을 정리해, 로컬 경로로 남지 않도록 했습니다.
- 공유 링크 화면에 "링크 복사" 버튼을 추가해 수행자/의뢰자에게 전달할 URL을 즉시 클립보드에 복사할 수 있도록 했습니다.
- PDF 첨부 기능(업로드, 미리보기, 병합)을 전면 제거하고 관련 UI/엔드포인트를 정리했습니다.
