# Generated by Django migration for final meetup order number format update

from django.db import migrations
import uuid


def update_meetup_order_number_format_final(apps, schema_editor):
    """밋업 주문번호를 최종 형식으로 업데이트: store_id-ticket-YYYYMMDD-해시값 (밋업 진행일 기준)"""
    ...
    for order in meetup_orders:
        try:
            # 밋업 진행일을 기준으로 하되, 없으면 생성일 사용
            if order.meetup and order.meetup.date_time:
                base_date = order.meetup.date_time
            else:
            ...
            new_order_number = f"{store_id}-ticket-{date_str}-{hash_value}"
...
def reverse_meetup_order_number_format_final(apps, schema_editor):
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('meetup', '0014_update_order_number_to_new_format'),
    ]

    operations = [
        migrations.RunPython(
            update_meetup_order_number_format_final,
            reverse_meetup_order_number_format_final,
        ),
    ] 